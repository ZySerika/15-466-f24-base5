...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const {
    ...
    // Convert the 'step' into a barycentric coordinates velocity:
    glm::vec3 step_coords = to_world_point(start) + step;     // Project 'step' in world space to final point
    glm::vec3 end_weights = barycentric_weights(a, b, c, step_coords); // Convert to barycentric coordinates
    glm::vec3 v = end_weights - start.weights;   // Barycentric velocity
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	glm::uvec2 reverse_edge(start.indices.y, start.indices.x);

	auto it = next_vertex.find(reverse_edge);
	...
	if (it != next_vertex.end()) {
		...
		//TODO
		// Update 'end' with the indices of the neighboring triangle
        end.indices.x = start.indices.y;      // Keep the order consistent with the edge
        end.indices.y = start.indices.x;      // Revert the edge order in 'end'
        end.indices.z = it->second;           // The third vertex of the neighboring triangle

		// Update 'end' barycentric weights (weights.z remains zero on the edge)
        end.weights.x = start.weights.y;      // Swap the weights corresponding to the swapped indices
        end.weights.y = start.weights.x;
        end.weights.z = 0.0f;

		const glm::vec3 &a0 = vertices[start.indices.x];
        const glm::vec3 &b0 = vertices[start.indices.y];
        const glm::vec3 &c0 = vertices[start.indices.z];
        glm::vec3 normal0 = glm::normalize(glm::cross(b0 - a0, c0 - a0));

        const glm::vec3 &a1 = vertices[end.indices.x];
        const glm::vec3 &b1 = vertices[end.indices.y];
        const glm::vec3 &c1 = vertices[end.indices.z];
		...
		//TODO
		// Compute the rotation quaternion from the starting normal to the ending normal
        rotation = glm::rotation(normal0, normal1);
